...
public class JobDriver_BuyItem : JobDriver
{
    ...
    public override IEnumerable<Toil> MakeNewToils()
    {
        ...
        if (TargetThingA != null)
        {
            ...
            var duration = Rand.Range(MinShoppingDuration, MaxShoppingDuration);
    ...
    private void BuyThing(Toil toil)
    {
        ...
        var maxSpace = toil.actor.GetInventorySpaceFor(Item);
        var inventory = toil.actor.inventory.innerContainer;

        var silver = inventory.FirstOrDefault(i => i.def == ThingDefOf.Silver);
        ...
        var count = Mathf.Min(Item.stackCount, maxSpace, maxWanted);
