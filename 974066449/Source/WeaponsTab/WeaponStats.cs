...
namespace WeaponStats
{
    public class MainTabWindow_WeaponStats : MainTabWindow
    {
        ...
        private void DoTurretsPage(Rect rect, int count, List<Turret> turretList)
        {
            colDef[] turretHeaders =
            {
                new colDef("HP", "hp"),
                new colDef("Value", "marketValue"),
                new colDef("Damage", "damage"),
            ...
            ww += LABEL_WIDTH;
            foreach (colDef h in turretHeaders)
            {
                printCellSort(WeaponsTab.Turrets, h.property, h.label, ww);
                ww += STAT_WIDTH;
            }
            
            if (!this.ce)
            {
                if (accRbAll) printCellHeader(WeaponsTab.Turrets, "accuracy", "WeaponStats.ColAccuracy".Translate(), ww, ACCURACY_WIDTH);
                else if (accRbTouch) printCellSort(WeaponsTab.Turrets, "accuracyTouch", "WeaponStats.ColAccuracy".Translate(), ww, ACCURACY_WIDTH);
                else if (accRbShort) printCellSort(WeaponsTab.Turrets, "accuracyShort", "WeaponStats.ColAccuracy".Translate(), ww, ACCURACY_WIDTH);
                else if (accRbMedium) printCellSort(WeaponsTab.Turrets, "accuracyMedium", "WeaponStats.ColAccuracy".Translate(), ww, ACCURACY_WIDTH);
                else if (accRbLong) printCellSort(WeaponsTab.Turrets, "accuracyLong", "WeaponStats.ColAccuracy".Translate(), ww, ACCURACY_WIDTH);
                ww += ACCURACY_WIDTH;
                ww += DTYPE_WIDTH;
            }
            rect.y -= 35;
            ...
            {
                DrawTurretRow(t, num, inRect.width);
                num++;
            }
        ...
        private int DrawCommonButtons(int x, int rowNum, float rowWidth, Thing t, Weapon w)
        {
            ...
            if (w.craftable)
            {
                if (w.cratablePos != null)
                {
                    if (Widgets.ButtonInvisible(new Rect(20, ROW_HEIGHT * rowNum, rowWidth, ROW_HEIGHT)))
                    {
                        var gti = new RimWorld.Planet.GlobalTargetInfo(w.cratablePos);
                        CameraJumper.TryJumpAndSelect(gti);
                    }
                }

                var pawnIcon = ContentFinder<Texture2D>.Get("UI/Icons/Craftable", true);
                var p = new Rect(0, ROW_HEIGHT * rowNum + (ROW_HEIGHT - pawnIcon.height) / 2,
            ...
            else if (w.inStorage)
            {
                if (Widgets.ButtonInvisible(new Rect(20, ROW_HEIGHT * rowNum, rowWidth, ROW_HEIGHT)))
                {
                    var gti = new RimWorld.Planet.GlobalTargetInfo(w.storagePos, Find.CurrentMap);
                    CameraJumper.TryJumpAndSelect(gti);
                }

                var storIcon = ContentFinder<Texture2D>.Get("UI/Icons/Storage", true);
                var p = new Rect(0, ROW_HEIGHT * rowNum + (ROW_HEIGHT - storIcon.height) / 2,
            ...
            else
            {
                if (Widgets.ButtonInvisible(new Rect(20, ROW_HEIGHT * rowNum, rowWidth, ROW_HEIGHT)))
                {
                    var gti = new RimWorld.Planet.GlobalTargetInfo(t);
                    CameraJumper.TryJumpAndSelect(gti);
                }
        ...
        private void DrawRangedRow(RangedWeapon t, int num, float w)
        {
            ...
            else
            {
                ...
                ww += STAT_WIDTH;
                printCell(Math.Round(t.maxRange, 1), num, ww);
                ww += STAT_WIDTH;
                printCell(Math.Round(t.cooldown, 2), num, ww);
                ww += STAT_WIDTH;
                printCell(Math.Round(t.warmup, 2), num, ww);
                ww += STAT_WIDTH;
        ...
        private void DrawTurretRow(Turret t, int num, float w)
        {
            DrawCommon(num, w);
            int ww = 0;
            ww = this.DrawCommonButtons(ww, num, w, t.thing, t);
        ...
        private void UpdateListSorting()
        {
            ...
            pir = typeof(Turret).GetProperty(this.sortProperty);
            if (pir != null)
            {
                turretList = this.sortOrder == "DESC"
                    ? turretList.OrderByDescending(o => pir.GetValue(o, null)).ToList()
                    : turretList.OrderBy(o => pir.GetValue(o, null)).ToList();
            }
        }
        ...
        private void UpdateList()
        {
            ...
            if (this.showGround)
            {
                ...
                foreach (var th in Find.CurrentMap.listerThings.ThingsInGroup(ThingRequestGroup.Apparel))
                {
                    if (!th.Position.Fogged(Find.CurrentMap))
                    {
            ...
            {
                var recipesDict = new Dictionary<string, bool>();
                foreach (var building in buildingList)
                {
                    ...
                    foreach (var recipe in recipeList)
                    {
                        ...
                        if (thingDef == null || recipesDict.ContainsKey(thingDef.defName)) continue;
                        recipesDict[thingDef.defName] = true;
                ...
                {
                    if (t.building == null || !t.BuildableByPlayer || !t.IsResearchFinished || (!t.building.IsTurret && !t.building.IsMortar)) continue;
                    tmpTurret = new Turret();
                    if (t.MadeFromStuff)
                    {
                        var stuff = GenStuff.DefaultStuffFor(t);
                        tmpTurret.fillFromThing(ThingMaker.MakeThing(t, stuff));
                    }
                    else
                    {
                        tmpTurret.fillFromThing(ThingMaker.MakeThing(t));
                    }

                    tmpTurret.craftable = true;
                    turretList.Add(tmpTurret);
                    turretCount++;
                }
            }
            
            foreach (var building in buildingList.Where(building => (building.def.building.IsTurret || building.def.building.IsMortar)))
            {
                tmpTurret = new Turret();
                tmpTurret.fillFromThing(building);
                turretList.Add(tmpTurret);
                turretCount++;
            }
        ...
        public override void DoWindowContents(Rect rect)
        {
            ...
            list.Add(new TabRecord("WeaponStats.Turrets".Translate(),
                delegate { this.curTab = WeaponStats.MainTabWindow_WeaponStats.WeaponsTab.Turrets; }, this.curTab == WeaponsTab.Turrets));
            ...
            if (this.curTab != WeaponsTab.Turrets)
            {
            ...
            switch (wpTab)
            {
                ...
                case WeaponsTab.Apparel:
                    ...
                    break;
                case WeaponsTab.Turrets:
                    DoTurretsPage(rect, turretCount, turretList);
                    break;
                default:
                    DoRangedPage(rect, rangedCount, rangedList);
                    break;
